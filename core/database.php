...
class Database
{
    ...
    static public $database = "applingi_topreal";
    static public $username = "applingi_admin";
    static public $password = "QAZwsxedc123";
    static public $link;
    ...
    static function Register($login, $email, $name, $password)
    {
        if (Database::ConnectToDatabase())
        {
            $query = "call Register('$login', '$email', '$name', '$password');";
            $results = mysql_query($query);
            $row = mysql_fetch_row($results);
            mysql_close(Database::$link);
            return $row[0];
    ...
    static function GetPriceCharacteristics($offerId)
    {
        if (Database::ConnectToDatabase())
        {
            $index = 0;
            $query = "call GetPriceCharacteristics($offerId);";
            $results = mysql_query($query);
            while ($result = mysql_fetch_array($results, MYSQL_NUM))
            {
                $characteristics[$index][0] = $result[0];
                $characteristics[$index][1] = $result[1];
                $characteristics[$index][2] = $result[2];
                $index++;
            }
            mysql_close(Database::$link);
            return $characteristics;
