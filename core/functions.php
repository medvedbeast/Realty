...
if (isset($_REQUEST["function"]))
{
    switch ($_REQUEST["function"])
    {
        ...
        case "Register":
            {
                echo Database::Register($_REQUEST["login"], $_REQUEST["email"], $_REQUEST["name"], $_REQUEST["password"]);
                break;
            }
        case "GetPaidOffers":
            {
        ...
        case "CreateRegistrationMail":
            {
                CreateRegistrationMail($_REQUEST["login"], $_REQUEST["email"], $_REQUEST["name"], $_REQUEST["password"]);
                break;
            }
    }
}
...
function GetOfferPreviewsHTML($data)
{
    ...
    {
        $characteristics = Database::GetPriceCharacteristics($offer->id);
        ...
        if ($_REQUEST["ownerId"] == 0)
        {
            ...
            <div class="result shadowed <?= $index == 0 ? "left" : ($index == $previewsPerRow - 1 ? "right" : ""); ?>">
                <div class="header">
                    <?= "Объявление №$offer->id" ?>
                </div>
                <div class="content">
...
function GetOfferHTML($data)
{
    ...
    Debug::Log(print_r("<br/>" . $images . "<br/>"));
...
function CreateRegistrationMail($login, $email, $name, $password)
{
    $message = @"
        <html>
            <head></head>
            <body>
                <p><h1>ПОДТВЕРЖДЕНИЕ РЕГИСТРАЦИИ</h1></p>
                <p><b>$name</b>, Вы получили это письмо, так как Ваш электронный адрес - <b>$email</b> - был указан при регистрации на сайте <a href='http://top-real.com.ua'>top-real.com.ua</a>.</p>
                <p>Что бы завершить регистрацию, перейдите, пожалуйста, по адресу:</p>
                </hr>
                <p><a href='http://top-real.com.ua/login.php?confirm=" . $login . "&email=$email&name=$name&password=$password'>ЗАВЕРШЕНИЕ РЕГИСТРАЦИИ</a></p>
                </hr>
                Это сообщение отправлено роботом. Пожалуйста, не отвечайте на него.
            </body>
        </html>";
    $headers = "From: registration@top-real.com.ua\r\nMIME-Version: 1.0\r\nContent-type: text/html; charset=UTF-8\r\nX-Mailer: PHP" . phpversion();
    mail($email, "Подтверждение регистрации на top-real.com.ua", $message, $headers);
}